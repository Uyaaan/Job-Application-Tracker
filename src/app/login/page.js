...
// --- SUBMIT BUTTON COMPONENT (Reused) ---
...
export default function AuthPage() {
  ...
  // STATE: Controls the Slide Animation (false = Login, true = Register)
  const [isSignUp, setIsSignUp] = useState(false);

  // --- ACTIONS ---
  // We need separate states for Login and Register forms
  ...
  // --- SUCCESS REDIRECTS ---
  useEffect(() => {
    if (loginState.success) router.push("/dashboard");
  }, [loginState.success, router]);

  useEffect(() => {
    if (registerState.success) {
      // After successful register, switch to login view and show success message (optional)
      setIsSignUp(false);
      // In a real app, you might auto-login here or show a toast
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center bg-gray-100 p-4 font-sans">
      ...
      <div className="relative bg-white rounded-3xl shadow-2xl overflow-hidden w-full max-w-5xl min-h-[600px] md:min-h-[700px]">
        {/* --- FORM CONTAINER: SIGN UP (Register) --- */}
        {/* Sits on the right when active */}
        <div
          className={`absolute top-0 h-full w-full md:w-1/2 transition-all duration-700 ease-in-out bg-white z-10
            ${
              isSignUp
                ...
                : "md:translate-x-full opacity-0 invisible" // Hide when not active
            }`}
        >
        ...
        >
          <div className="h-full flex flex-col justify-center p-8 md:p-12 lg:p-16">
            ...
            <form action={loginDispatch} className="space-y-4">
              ...
              {/* Login Button with Lockout Logic */}
        ...
        >
          ...
          >
            ...
            {/* PANEL: RIGHT (Shown during Register, encourages Login) */}
            {/* When isSignUp is TRUE, this is visible on the left side of the screen */}
            <div className="absolute top-0 left-0 w-1/2 h-full flex flex-col items-center justify-center text-center px-12 text-white">
              {/* Background Image for Register State */}
              <img
              ...
              <div className="relative z-10">
                ...
                <p className="mb-8 text-lg text-blue-100">
                  Login to track your applications and view your progress.
                </p>
                <button
                  onClick={() => setIsSignUp(false)}
...
// Separate component for the Login Button to handle FormStatus + Timer
